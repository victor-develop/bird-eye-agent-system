...
这些任务之间没有强依赖关系，可以同时启动多个 TaskRunner：
...
**注意**：问题排查任务只能并行到"发现问题"这一步，具体修复方案需要您 review。

#### 必须线性执行的任务

这些任务存在依赖关系或需要人工 review，必须按顺序执行：

- **代码 review 相关的任务**
  - 任何涉及修改代码的任务
  - 因为您需要逐个 review 每个 TaskRunner 的输出
  - Review 通过后才能启动下一个任务
...
使用 `@treeWork populate <task_id>` 批量准备任务，然后：
...
**关键原则**：
- 人类作为调度者，决定哪些任务可以并行
- 并行任务执行期间，您可以在多个会话间切换 review
- `@treeWork receive` 支持批量同步，简化进度更新
